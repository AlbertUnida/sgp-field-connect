...
### Alta prioridad
4. ✅ **Refactor de ClienteDetalle.tsx y Admin.tsx — COMPLETO (2026-07-11 / 2026-07-21).**
   - **ClienteDetalle (2026-07-11):** `src/components/cliente/`: `types.ts`, `InfoRow`, `FotoGestion` + `CobroLocalForm`, `CobroEventosForm`, `EventoForm`, `GestionForm`. Bajó **2370 → 1696 líneas (−28%)**.
   - **Admin (2026-07-21):** las 6 secciones extraídas a `src/components/admin/`: `EquipoSection` (híbrido: recibe `ejecutivos` + `onReload` por prop), `CensoSection` (presentacional, props del padre), `CatalogoSection`, `EventosSection`, `ResultadosSection` (autocontenidas, cargan sus datos), `SeguimientoSection` (recibe `ejecutivos` por prop). Admin.tsx bajó **1534 → 286 líneas (−81%)**. Sin cambios de comportamiento (tsc en 0). Componentes presentacionales/autocontenidos; el padre conserva ejecutivos/CENSO. **Falta que el usuario click-testee cada tab (esp. EQUIPO: crear usuario Rol+Área, editar nombre/rol/área inline, metas) y commitee.**

18. **Hardening de RLS (defensa en profundidad) — PENDIENTE, analizado 2026-07-21.** La RLS actual tiene dos niveles de laxitud, con costo/riesgo muy distinto:
   - **Camino A — apretar SELECT (medio día, RIESGOSO):** hoy `clientes`, `cobros`, `eventos_agenda`, `profiles`, `historial` tienen SELECT abierto (`auth.role()='authenticated'`). La app depende de esto a propósito (tab "Todos", COBRANZAS compartida, Reportes, Dashboard gerencial, Monitoreo, Seguimiento leen la cartera de todos). Cerrarlo obliga a replicar en SQL la regla "lectura abierta / escritura restringida" y testear pantalla por pantalla. No hacer sin ventana de pruebas.
   - **Camino B — apretar ESCRITURA (rápido, BAJO RIESGO, recomendado hacer primero):** 1 migración, no toca el modelo de lectura. Agujeros concretos: `Registrar gestiones` permite insertar con **cualquier `ejecutivo_id`** (spoofing de atribución) → forzar `ejecutivo_id = auth.uid()` o manager; `update_eventos_agenda` es `USING(true) WITH CHECK(true)` → cualquiera edita cualquier evento (redundante con `eventos_update`, que ya tiene la regla correcta) → **DROP**; `manage_rubros_evento`/`manage_tipos_evento` con `USING(true)` → cualquier autenticado modifica el catálogo → restringir a admin. `Crear clientes`/`Registrar cobros` sin check de pertenencia (menor: el flujo real va por RPC).
