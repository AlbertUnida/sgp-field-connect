...
### Alta prioridad
4. ✅ **Refactor de ClienteDetalle.tsx — FASES 0 y 1 COMPLETAS (2026-07-11)**, probado por el usuario. `src/components/cliente/`: `types.ts`, `InfoRow`, `FotoGestion` (Fase 0) + `CobroLocalForm`, `CobroEventosForm`, `EventoForm`, `GestionForm` (Fase 1). ClienteDetalle bajó **2370 → 1696 líneas (−28%)**, sin cambios de comportamiento (tsc en 0, build OK); componentes presentacionales, el estado/lógica quedan en el padre. Lo que resta en el archivo es render de solo lectura (bajo riesgo). **Pendiente aparte: Admin.tsx (1.511 líneas) sigue intacto** — se puede aplicar el mismo enfoque cuando convenga.
