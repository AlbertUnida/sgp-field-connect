...
1. **Setup de web push (usuario)** — el código está completo; faltan los pasos de infraestructura del usuario: SQL de `push_suscripciones`, claves VAPID, secrets, deploy de la Edge Function `enviar-alertas` y cron. Pasos exactos en CLAUDE.md → "Setup web push". Verificar después de configurado: activar desde Perfil → Notificaciones y disparar la función manualmente con el CRON_SECRET para probar.
